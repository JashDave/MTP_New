...
namespace kvstore {
  ...
  public:
    ...
    RamCloud *cluster;
    // vector<string> v;
    // vector<MultiReadObject*> vget;
    // vector<MultiWriteObject*> vput;
    // vector<MultiRemoveObject*> vdel;
  };
...
bool getTableId(string tablename,RamCloud *cluster,string opr, vector<uint64_t> &vt, vector<string> &v);
  ...
  void KVRequest::get(KeyType const& key,string tablename){
    ...
    if(getTableId(tablename,c_kvsclient->cluster,"get",vgett,v)){
    v.push_back("get");
    vget.push_back(skey);
  }
  }
  ...
  void KVRequest::put(KeyType const& key,ValType const& val,string tablename){
    ...
    if(getTableId(tablename,c_kvsclient->cluster,"put",vputt,v)){
    v.push_back("put");
    vputk.push_back(skey);
    vputv.push_back(sval);
  }
  }
  ...
  void KVRequest::del(KeyType const& key,string tablename){
    ...
    if(getTableId(tablename,c_kvsclient->cluster,"del",vdelt,v)){
    v.push_back("del");
    vdel.push_back(skey);
  }
  }
