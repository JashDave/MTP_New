    ...
    for(int i=0; i<THREAD_COUNT; i++){
      td[i] = thread([&,i]{
        ...
        while(run){
          ...
          r2 = rng.zipf();
          // r2 = rng.uniform();
          ks[id].get<string,string>(keys[r2],TABLE);
          ...
          ks[id].put<string,string>(keys[r2],vals[r2],TABLE);
          kr = ks[id].execute();
          ks[id].reset();
          kd = kr.get<string>(0);
          if(kd.ierr != 0){
      ...
      for(int i=0; i<THREAD_COUNT; i++){
        td[i] = thread([&,i]{
          ...
          while(run){
            r2 = rng.zipf();
            // r2 = rng.uniform();
            ks[id].sget<string,string>(keys[r2],TABLE);
            KVResultSet kr = ks[id].execute();
            ks[id].reset();
            kd = kr.get<string>(0);
            if(kd.ierr != 0){
            ...
            ks[id].sput<string,string>(keys[r2],vals[r2],TABLE);
            kr = ks[id].execute();
            ks[id].reset();
            kd = kr.get<string>(0);
            if(kd.ierr != 0){
