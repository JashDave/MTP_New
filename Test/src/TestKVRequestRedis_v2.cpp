...
void doSomething(int tid){
  ...
  kr.bind("10.129.28.141:7003");//Invoked exactly once
  ...
  rs = kr.execute();
   if(rs.get<string>(0).ierr!=0 || rs.get<double>(1).ierr!=0){
     cout<<"Error in tid "<<tid<<" "<<rs.get<string>(0).serr<<" "<<rs.get<double>(1).serr<<endl;
   } else {
     cout<<"All good in tid "<<tid<<" "<<rs.get<string>(0).value<<" "<<rs.get<double>(1).value<<endl;
   }
   kr.reset();
